...
class MyApp extends StatelessWidget { //extends를 통해서 상속
  const MyApp({super.key});

  @override
  Widget build(BuildContext context) { //모든 위젯들이 공통적으로 가지는 메서드(함수)
    //BuildContext : 모든 위젯이 가지고 있는 고유의 아이디값.
    // 모든 위젯들이 트리 형태로 구성돼 있을 때 각 위젯의 위치가 BuildContext가 되어 각 위젯마다 갖고 있게 됨

    return MaterialApp( //MaterialApp이라는 메서드(함수)의 프로퍼티(하위내용) - title, theme, home
      title: 'Flutter Demo',
      theme: ThemeData(
...
class MyHomePage extends StatelessWidget { //StatelessWidget을 상속받음
  ...
  Widget build(BuildContext context){
    ...
    return Scaffold( //Scaffold: 앱의 화면이 기본적으로 갖출 기능들을 선언해둔 위젯
      appBar: AppBar(
        title: Text(
          title,
        ),
      ...
      body: Container( /// Container 위젯으로 감싸는 이유 : padding, color(배경색), width&height 프로퍼티가 가능
        /// (Text 위젯에는 padding 프로퍼티처럼 여백을 주는 기능이 없음)
        /// (Padding 위젯도 있음. 하지만 Padding 위젯은 프로퍼티로 padding과 child만이 가능)
        child: Text('Hello, Flutter~'), //Container은 child가 필수인 위젯
